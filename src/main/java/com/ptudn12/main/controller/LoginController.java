...
public class LoginController {
	...
	private void handleForgotPassword() {
		...
		if (result.isPresent()) {
			...
			task.setOnSucceeded(e -> {
				if (task.getValue()) {
					showAlert(Alert.AlertType.INFORMATION, "Thành công", "Mật khẩu mới đã được gửi về email của bạn!\n"
							+ "Vui lòng kiểm tra email (kể cả thư mục Spam).");
				} else {
					showAlert(Alert.AlertType.ERROR, "Lỗi", "Không thể khôi phục mật khẩu!\n"
	...
	private boolean processPasswordReset(String maNhanVien) {
		try {
			...
			// Gửi email
			boolean emailSent = EmailService.sendPasswordResetEmail(nhanVien.getEmail(), newPassword,
